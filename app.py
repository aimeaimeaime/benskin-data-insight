...
# # --- Logique de style pour le tableau ---
# def fatigue_color(val):
#     if val > 7: return "#ef4444"
#     elif val > 4: return "#f59e0b"
#     return "#22c55e"
...
# def submit():
#     try:
#         # Correspondance exacte avec ton CREATE TABLE + ALTER TABLE
#         data = {
#             "age": int(request.form.get("age") or 0),
#             "experience": int(request.form.get("experience") or 0),
#             "heures_travail": float(request.form.get("heures_travail") or 0),
#             "clients": int(request.form.get("clients") or 0),
#             "revenu": float(request.form.get("revenu") or 0),
#             "fatigue": int(request.form.get("fatigue") or 1),
#             "accident": request.form.get("accident") == "1",
#             # Colonnes issues de ton ALTER TABLE
#             "distance_km": float(request.form.get("distance_km") or 0),
#             "carburant_litre": float(request.form.get("carburant_litre") or 0),
#             "note_client": float(request.form.get("note_client") or 3.0),
#             "zone": request.form.get("zone") or "Inconnue"
#         }
#         supabase.table("conducteurs").insert(data).execute()
#         return redirect("/dashboard")
#     except Exception as e:
#         return f"Erreur d'enregistrement (Vérifie Supabase) : {str(e)}"
...
#         # Sécurisation des colonnes NULL (Anciennes lignes)
#         df["distance_km"] = pd.to_numeric(df.get("distance_km"), errors="coerce").fillna(0.0)
#         df["carburant_litre"] = pd.to_numeric(df.get("carburant_litre"), errors="coerce").fillna(0.0)
#         df["note_client"] = pd.to_numeric(df.get("note_client"), errors="coerce").fillna(3.0)
#         df["zone"] = df.get("zone").fillna("Inconnue")
...
#         # ── 3. PCA
#         scaler = StandardScaler()
#         feat_pca = ["age", "experience", "heures_travail", "clients", "revenu", "fatigue", "distance_km", "carburant_litre", "note_client"]
#         scaled_pca = scaler.fit_transform(df[feat_pca])
#         pca = PCA(n_components=2)
#         pca_result = pca.fit_transform(scaled_pca)
...
#         X_clf = df[["fatigue", "experience", "age", "heures_travail", "distance_km"]]
#         y_clf = df["accident"].astype(int)
#         clf_model = RandomForestClassifier(n_estimators=50, random_state=42).fit(X_clf, y_clf)
#         df["risque_score"] = clf_model.predict_proba(X_clf)[:, 1] * 100
#         feature_importance = dict(zip(X_clf.columns, [round(v * 100, 1) for v in clf_model.feature_importances_]))

#         # ── 5. CLASSIFICATION NON-SUPERVISÉE (K-Means)
#         feat_km = ["revenu", "fatigue", "clients", "note_client"]
#         df["cluster"] = KMeans(n_clusters=3, n_init=10, random_state=42).fit_predict(scaler.fit_transform(df[feat_km]))
#         df["cluster_label"] = df["cluster"].map({0: "Performant", 1: "Modéré", 2: "À risque"})
...
#         # ── Données graphiques (Clés originales)
#         df_plot = df.sort_values(by="heures_travail")
#         graph_data = {
#             "heures": df_plot["heures_travail"].tolist(),
#             "revenu": df_plot["revenu"].tolist(),
...
#         return render_template("dashboard.html", graph_data=json.dumps(graph_data), table_data=rows.to_dict(orient="records"), stats=stats)
